...
namespace AlpacaDashboard
{
    ...
    public partial class AlpacaDashboard : Form
    {
        ...
        private Broker PaperBroker { get; set; }
        private Broker LiveBroker { get; set; }
        ...
        private async void AlpacaDashboard_Load(object sender, EventArgs e)
        {
            ...
            foreach (Type _type in _bots)
            {
                ...
                {
                    wlLive = await LiveBroker.CreateWatchList(_type.Name, new string[] { });
                }
                ...
                {
                    wlPaper = await PaperBroker.CreateWatchList(_type.Name, new string[] { });
                }
                ...
                ToolStripMenuItem tsi = new ToolStripMenuItem("Add to " + _type.Name);
                tsi.Tag = instances;
            ...
            await LoadBotDetails(Environment);
            ...
            foreach (Type _type in _scanners)
            {
                ...
                btn.Name = "textBox" + _type.Name + "Scan";
                btn.Text = "Scan";
                btn.BackColor = Color.Gray;
                tableLayoutPanel.Controls.Add(btn);
                btn.Click += ScanButton_Click;
            }
            await LoadScannerDetails(Environment);
            ...
            {
                Stock.SubscribeMinutesBarForAllSymbols(LiveBroker, PaperBroker);
            }
        ...
        /// </summary>
        /// <returns></returns>
        private async Task PaperPfolioEnableEvents()
        {
            //event to receive price updates
        ...
        /// </summary>
        /// <returns></returns>
        private async Task LivePfolioEnableEvents()
        {
            //event to recive open order updates
        ...
        private void UpdateListViewClosedOrders(IReadOnlyCollection<IOrder> closedOrders)
        {
            try
            {
                ...
                {
                    ListViewItem item = new ListViewItem(co.Symbol);
                    var tz = co.FilledAtUtc == null ? "" : TimeZoneInfo.ConvertTimeFromUtc((DateTime)co.FilledAtUtc, easternZone).ToString();
                    var ord = $"{co.OrderType.ToString()} { co.OrderSide.ToString()} @ {co.LimitPrice.ToString()} {tz}";
        ...
        private async void buttonOrderConfirm_Click(object sender, EventArgs e)
        {
            ...
            if (Environment == "Live")
            {
                IOrder order = await LiveBroker.SubmitOrder(orderSide, orderType, timeInForce, extendedHours, textBoxSymbol.Text, orderQuantity, stopPrice, limitPrice, trialOffsetPercentage, trailOffsetDollar);
                //since no onTrade event generated for Accepted status
                if (order != null && order.OrderStatus == OrderStatus.Accepted) LiveBroker.UpdateOpenOrders();
            }
            if (Environment == "Paper")
            {
                IOrder order = await PaperBroker.SubmitOrder(orderSide, orderType, timeInForce, extendedHours, textBoxSymbol.Text, orderQuantity, stopPrice, limitPrice, trialOffsetPercentage, trailOffsetDollar);
                //since no onTrade event generated for Accepted status
                if (order != null && order.OrderStatus == OrderStatus.Accepted) PaperBroker.UpdateOpenOrders();
            }
        }
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void Instance_ScannerListUpdated(object? sender, EventArgs e)
        {
            ...
            instance.Broker.UpdateWatchList(instance.watchList, symbols);
        ...
        private async Task LoadScannerDetails(string env)
        {
            try
            {
                if (Environment == env)
        ...
        private static void DisplayControlsForScanner(IScanner instance, TableLayoutPanel tableLayoutPanel, Type _type, PropertyInfo p)
        {
            ...
            if (p.PropertyType == typeof(BarTimeFrameUnit))
            {
        ...
        private static void GenerateControlForScanner(Type _type, IScanner instance, TableLayoutPanel tableLayoutPanel, PropertyInfo p)
        {
            if (p.PropertyType == typeof(decimal))
            {
                ...
                tb.Name = "textBox" + _type.Name + p.Name;
                tb.Text = p.GetValue(instance).ToString();
            ...
            if (p.PropertyType == typeof(BarTimeFrameUnit))
            {
                ...
                cb.DataSource = Enum.GetValues(typeof(BarTimeFrameUnit));
                cb.Name = "comboBox" + _type.Name + p.Name;
                cb.Text = p.GetValue(instance).ToString();
        ...
        private void LoadBotListView(IBot instance, ListView lv, Dictionary<string, IPosition> listOfSymbolPosition)
        {
            try
            {
                if (listOfSymbolPosition != null)
                {
                    ...
                    {
                        ListViewItem item = new ListViewItem(assetPosition.Key);
                        try
                        {
                            CancellationTokenSource cts = null;
                            instance.ActiveSymbols.TryGetValue(assetPosition.Key, out cts);
                            if (cts != null)
                            {
            ...
            if (hit.SubItem != null)
            {
                ...
                if (e.Button == MouseButtons.Right)
                {
                    if (focusedItem != null && focusedItem.Bounds.Contains(e.Location))
                    {
                        ...
                        CancellationTokenSource cts = null;
                        instance.ActiveSymbols.TryGetValue(symbol, out cts);
                        if (cts != null)
                        {
        ...
        private void Instance_BotListUpdated(object? sender, EventArgs e)
        {
        ...
        private async Task LoadBotDetails(string env)
        {
            try
            {
                if (Environment == env)
